...
//------------------------------------BUY:
   if(inters==1) //for BUY: Check Indicator
     {
      // Print("BUY:  ", inters);
      //check for number of positions // for Buy: Check Money Management

      if(BUYorders<Buymax)
        {
         if(BUYorders==0)
           {
            OrderSend(Symbol(),OP_BUY,tradeVol,Ask,0,StopYes*(CurrentAskPrice-UsePoint*SLone),CurrentAskPrice+UsePoint*TPone,
                      "Buy Order",0,0,Green);
           ...
           {
            double lastBuyPrice=lastBuyOrderAskPrice();
            double diffPip=(lastBuyPrice-Ask)/UsePoint;

            if(MathAbs(diffPip>Buydist))
              {
               OrderSend(Symbol(),OP_BUY,tradeVol,Ask,0,StopYes*(CurrentAskPrice-UsePoint*SLone),CurrentAskPrice+UsePoint*TPone,
                         "Buy Order",0,0,Green);
   ...
   if(inters==2) //for Sell: Check Indicator
     {
      ...
      //check for numer of positions // for Sell: Check Money Management
      if(SELLorders<Sellmax)
        {
   
         if(SELLorders==0)
           {
 
            OrderSend(Symbol(),OP_SELL,tradeVol,Bid,0,StopYes*(CurrentAskPrice+UsePoint*SLone),CurrentAskPrice-UsePoint*TPone,
                      "Buy Order",0,0,Red);
            Print("Hello: 3");
         ...
         else 
           {
            ...
            double lastSellPrice=lastSellOrderAskPrice();
            double diffPip=(lastSellPrice-Ask)/UsePoint;
            Print("LASTTTTTTTT: ",diffPip);
            if(MathAbs(diffPip)>Selldist)
              {
               OrderSend(Symbol(),OP_SELL,tradeVol,Bid,0,StopYes*(CurrentAskPrice+UsePoint*SLone),CurrentAskPrice-UsePoint*TPone,
                         "Buy Order",0,0,Red);
               Print("Hello: pipDiff");
               Print(pipDiff);
               Print(MathAbs(pipDiff));
